# ISCE Caption Pipeline Repo Cleanup — Implemented Tasks (Parsed & Cleaned)
...
**Notes:** Audit 2025-12-14: Verified implementation and tests still pass (pytest -q).
...
Audit 2025-12-14: Verified implementation and tests still pass (pytest -q).
...
**Notes:** If additional lightweight dependencies are required for imports (e.g., numpy), add them to the install list and document in requirements-dev. Audit 2025-12-14: Reran targeted smoke/config suite (tests/test_config_loader.py, tests/test_stage2_smoke.py, tests/test_stage3_smoke.py, tests/test_e2e_smoke.py, tests/test_default_settings.py, tests/test_ui_job_runner_args.py); all passed under pytest -q and deliverables remain intact.
...
**Notes:** Audit 2025-12-14: Verified docs/ENTRYPOINTS.md enumerates the canonical chain/flags and AGENTS.md links to it; no changes needed.
...
**Notes:** Audit 2025-12-14: Confirmed experiments/ and legacy/ house the moved alternates with README pointers; pytest -q subset still clean.
...
**Notes:** Audit 2025-12-14: Reviewed run_pipeline.py strings and docs/CONFIGURATION.md for YAML override wording; no misleading pipeline_config.py claims remain and pytest -q subset stayed green.
...
**Notes:** Audit 2025-12-14: Confirmed DEFAULT_SETTINGS placeholders remain portable and test_default_settings.py covers resolution under temp dirs; pytest -q subset passed.
...
**Notes:** If you prefer not to add ruff, substitute with flake8; keep the test command explicit. Audit 2025-12-14: ruff.toml and scripts/lint.sh remain aligned and linting runs clean locally.
...
**Notes:** Audit 2025-12-14: Reran tests/test_ui_job_runner_args.py after import fix; mock-based argument construction passes and matches ui/backend/pipelines.py command assembly.
