...
def run_training(
    ...
    """Train the statistical model using programmatic inputs.

    This is a refactored version of the CLI workflow exposed via ``main``.  It is
    intended for programmatic callers (e.g., the web UI) that want to invoke the
    full training loop without shelling out to the script.  The implementation is
    identical to the CLI path but accepts keyword arguments instead of reading
    ``sys.argv``.

    Args:
        corpus: Directory containing ``*.json`` pre-enriched training files.
        constraints_path: Destination path for the generated ``constraints.json``.
        weights_path: Destination path for the trained ``model_weights.json``.
        config_path: Path to the feature configuration YAML file.
        iterations: Number of re-weighting passes to perform.
        error_boost_factor: Increment applied to the weight of misclassified
            samples after each iteration.

    Returns:
        A dictionary summarising the run, including the corpus files that were
        processed and the final accuracy reported during training.
    """

    corpus_paths = [str(p) for p in Path(corpus).glob("*.json")]
    if not corpus_paths:
        raise FileNotFoundError(f"No .json files found in corpus directory: {corpus}")
    ...
    return {
        ...
        "error_boost_factor": error_boost_factor,
...
def main():
    ...
    This script orchestrates the entire model training process by parsing CLI
    arguments and delegating to :func:`run_training`.
    """
