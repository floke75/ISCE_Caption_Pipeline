...
def test_transition_context_maxes_penalty_for_empty_second_line() -> None:
    constraints = _make_constraints()
    sliders = {"fragment_penalty": 9.0, "fragment_char_threshold": 5.0}
    cfg = _make_config()
    scorer = Scorer({}, constraints, sliders, cfg)

    token = {"w": "Wait", "pause_z": 0.0, "relative_position": 0.4, "pos": "VERB"}
    nxt = {"w": "what", "pos": "PRON"}
    row = TokenRow(token=token, nxt=nxt)
    ...
    assert pytest.approx(baseline["LB"] - penalised["LB"], rel=1e-6) == sliders["fragment_penalty"]
