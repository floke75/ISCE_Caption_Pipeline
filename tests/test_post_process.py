...
class TestReflowTokens(unittest.TestCase):
    ...
    def test_does_not_merge_across_speaker_change(self) -> None:
        """Merging should respect speaker boundaries."""
        ...
        tokens = [
            ...
            Token(w="We", start=0.3, end=0.6, speaker="B", break_type="LB"),
            Token(w="agree", start=0.6, end=1.0, speaker="B", break_type="SB"),
        ]

        cfg = _make_config()
        merged = reflow_tokens(tokens, MergeFriendlyScorer(), cfg)

        self.assertEqual([token.break_type for token in merged], ["SB", "LB", "SB"])
