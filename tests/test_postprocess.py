...
class TestReflowTokens(unittest.TestCase):
    """Regression tests for the ``postprocess`` helper."""
    ...
    def test_merges_short_block_with_following_block(self) -> None:
        ...
        cfg = _make_config()
        merged = postprocess(tokens, MergeFriendlyScorer(), cfg)
    ...
    def test_shifts_line_break_to_balance_lines(self) -> None:
        ...
        cfg = _make_config()
        balanced = postprocess(tokens, BalanceScorer(), cfg)
    ...
    def test_skips_merge_when_speaker_changes(self) -> None:
        ...
        # should not trap ``postprocess`` in an infinite loop.
