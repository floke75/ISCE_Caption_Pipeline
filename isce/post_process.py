    ...
    while start < len(refined):
        ...
        if (is_short or is_imbalanced) and end < len(refined) - 1:
            ...
            if next_tokens:
                ...
                next_lead = next_tokens[0]
                speakers_differ = (
                    boundary_token.speaker is not None
                    and next_lead.speaker is not None
                    and boundary_token.speaker != next_lead.speaker
                )
                # Respect diarization boundaries; never merge across speaker turns.
